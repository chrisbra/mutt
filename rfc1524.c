...
int rfc1524_mailcap_lookup(BODY *a, char *type, size_t typelen, rfc1524_entry *entry, int opt)
{
  ...
  while (!found && *curr)
  {
    ...
    muttdbg(2, "Checking mailcap file: %s", mutt_b2s(path));
 ...
 */
static void rfc1524_expand_filename(const char *nametemplate,
                                    const char *oldfile,
                                    BUFFER *newfile)
{
  int i, j, k, ps;
...
void mutt_rfc1524_expand_tmpdir_filename(const char *nametemplate,
                                         ...
                                         BUFFER *newfile)
{
  rfc1524_expand_filename(nametemplate, oldfile, newfile);
